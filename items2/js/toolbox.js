function toolbox(){
	...
	this.addZeros=function(number,zeros){
		var ret="";
		for(var i=0;i<zeros;i++){
			ret+="0";
			if(number<parseInt(1+ret)){
				for(var j=0;j<zeros-i-1;j++){
					number="0"+number;
				}
				return number;
			}
	...
	this.saveFile=function(data,name,extension){
	...
	this.saveImageFile=function(source,destination){
		var a = document.createElement('a');
	...
	this.saveJSONFile=function(text,filename){
	var a = document.createElement('a');
	var str=JSON.stringify(text);
	a.setAttribute('href','data:application/json;charset=utf-8,'+ encodeURIComponent(str));
	a.setAttribute('download',filename);
